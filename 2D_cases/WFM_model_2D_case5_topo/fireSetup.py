...
h   = 0.05   #kW/(m^3·K)
Tinf= 300.0 #K
RH  = 0.30   #aodim (relative humidity)
L   = 1.0   #m
...
#landscape properties         
        ...
        SC[l,m]=0.25*( np.sin(xc[l,m]/20.0)*np.cos(yc[l,m]/10.0-20.0) + np.sin(xc[l,m]/20.0-40.0)**2*np.sin(yc[l,m]/10.0)+2.0 ) #surface coverage
        Z[l,m]=50.0*np.exp(-((xc[l,m]-200)**2+(yc[l,m]-200)**2)/20000) #topography
        rhof[l,m]=400.0 #solid fuel density
        Cf[l,m]=1.00 #solid fuel specific heat
        H[l,m]=4000 #solid fuel heat power
        Mg[l,m]=0.0000 #green wood water content
