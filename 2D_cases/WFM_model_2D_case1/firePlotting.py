...
fig, ax = plt.subplots(1,2,figsize=(7,3.5))      #genera el objeto "figura" fig, ax = plt.subplots(1,2,figsize=(9,4.5)) ANTERIOR!!!!

levels = np.linspace(np.min(T), np.max(T), 12)
#print(levels)
plot1=ax[0].contour(Xp, Yp, ST, 6,levels=levels,colors="k",linewidths=0.5)  
...
plot1=ax[1].contour(Xp, Yp, SY, 6,colors="k",linewidths=0.5) 
...
fig.text(0.105, 0.89, "T_max="+str(round(np.max(ST),1))+" K", fontsize=7)
fig.text(0.105, 0.85, "T_min="+str(round(np.min(ST),1))+" K", fontsize=7)

fig.text(0.595, 0.89, "Y_max="+str(round(np.max(Y),1)), fontsize=7, color="white")
fig.text(0.595, 0.85, "Y_min="+str(round(np.min(Y),1)), fontsize=7, color="white")
